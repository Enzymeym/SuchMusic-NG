export default function convertEventDrivenPlugin(originalCode: string): string {
  ...
  return `/**
 * 由 SuchMusic 插件转换器转换 - @author sqj
 * @name ${pluginName}
 * @author ${author}
...
// 处理更新提示事件
function handleUpdateAlert(data, suchmusicApi) {
  // 每次运行脚本只能调用一次
  if (updateAlertSent) {
  ...
  // 通过 SuchMusic 的通知系统发送更新提示
  try {
    // 使用传入的 suchmusic API 对象发送通知
    if (suchmusicApi && suchmusicApi.NoticeCenter) {
      suchmusicApi.NoticeCenter('update', {
      ...
      console.error(\`[${pluginName}] SuchMusic API 不可用，无法发送更新提示\`);
    }
  } catch (error) {
...
function initializePlugin() {
  ...
  const mockLx = {
    ...
    on: (event, handler) => {
      console.log(\`[${pluginName + ' by Such 插件' || 'such插件'}] 注册事件监听器: \${event}\`);
      if (event === 'request') {
        requestHandler = handler;
      }
    },
    send: (event, data) => {
      console.log(\`[${pluginName + ' by Such 插件' || 'such插件'}] 发送事件: \${event}\`, data);
  ...
  // 注入到全局作用域
  // 注意：在 SuchMusicPluginHost 的沙箱中，global 和 globalThis 已经指向了 sandbox
  // 所以直接赋值给 global.lx 即可
  if (typeof global !== 'undefined') {
    global.lx = mockLx;
  } else if (typeof globalThis !== 'undefined') {
    globalThis.lx = mockLx;
  }
  ...
  try {
    ...
    // 我们将 mockLx 作为参数传入，命名为 lx，这样插件代码中的 lx 就能直接访问到
    ...
    isInitialized = true;
    console.log(\`[Such Music] 事件驱动插件初始化成功\`);
  } catch (error) {
    console.log(\`[Such Music] 插件初始化完成: \${error.message}\`);
    isInitialized = true;
  }
}
...
async function musicUrl(source, musicInfo, quality) {
  ...
  // 等待一小段时间让插件完全初始化（正常运行环境）
  // 在 SNOWDROP_SUMMARY 模式下跳过延迟，便于同步收集日志
  // if (!process.env.SNOWDROP_SUMMARY) {
  ...
  try {
    ...
    if (!finalUrl || typeof finalUrl !== 'string') {
      const errorMessage = \`获取 \${source} 音源链接失败: 返回结果格式无效\`;
      console.error(\`[\${pluginInfo.name}] Error: \${errorMessage}\`, result);
      throw new Error(errorMessage);
    }

    console.log(\`[\${pluginInfo.name}] Got URL: \${finalUrl}\`);
    return finalUrl;
  } catch (error) {
