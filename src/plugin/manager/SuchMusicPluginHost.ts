...
// import { MusicItem } from '../../main/ipc/musicSdk/type'
...
class SuchMusicPluginHost {
  ...
  private plugin: SuchMusicPlugin | null
   ...
   * @param logger 日志记录器
   */
  async loadPlugin(pluginPath: string, logger: Logger = console): Promise<SuchMusicPlugin> {
    try {
      ...
      this._initialize(logger)
      return this.plugin as SuchMusicPlugin
    } catch (error: any) {
