/***
 * 落雪插件兼容
 * 基于 SuchMusicPluginHost 实现并修改（原作者：时迁酱）
 * @description 提供洛雪插件和Such插件的兼容转换功能
 * @version 1.0.0
 * @author 时迁酱 & Enzymeym
 */
...
import SuchMusicPluginHost from './manager/SuchMusicPluginHost'
import Logger, { getLog } from './logger'
import {
  transformSnowdropPlugin,
  type SnowdropTransformResult
} from './utils/snowdrop-transform'
...
const pluginService = {
  async selectAndAddPlugin(type: 'lx' | 'cr') {
    try {
      ...
      const result = await dialog.showOpenDialog({
        title: `请选择你的 ${type == 'lx' ? '洛雪' : 'Such'} js插件`,
        filters: [
      ...
      let transformResult: SnowdropTransformResult | null = null

      // 插件格式校验
      if (type === 'cr') {
        // Such格式校验：检查是否包含suchmusic关键字
        if (!pluginCode.toLowerCase().includes('suchmusic')) {
          throw new Error('Such插件格式校验失败：代码有可能不是Such格式插件')
        }
      } else if (type === 'lx') {
        // 洛雪格式校验：检查是否包含lx关键字
      ...
      if (transformResult) {
        // 将转换过程中的日志统一输出到控制台，便于调试
        if (transformResult.logs.length) {
          console.group('[SnowdropTransform] 落雪插件本地安装日志')
          for (const log of transformResult.logs) {
            console.log(log)
          }
          console.groupEnd()
        }
        if (transformResult.warnings.length) {
          console.warn('落雪插件转换告警:', transformResult.warnings.join(' ; '))
        }
        if (transformResult.errors.length) {
          console.error('落雪插件转换错误:', transformResult.errors.join(' ; '))
        }
      }
  ...
  async addPlugin(pluginCode: string, pluginName: string) {
    try {
      // 首先解析插件信息
      const tempPluginManager = new SuchMusicPluginHost(pluginCode, new Logger('temp'))
      ...
      // 重新加载插件以确保正确初始化
      const suchPluginManager = new SuchMusicPluginHost()
      await suchPluginManager.loadPlugin(filePath, new Logger(pluginId))

      // 将插件添加到已加载插件列表
      loadedPlugins[pluginId] = suchPluginManager

      return {
        pluginId,
        pluginName: safePluginName,
        pluginInfo,
        supportedSources: suchPluginManager.getSupportedSources()
      }
    } catch (error: any) {
  ...
  getPluginById(pluginId: string): SuchMusicPluginHost | null {
      ...
      const results = await Promise.all(
        files.map(async (file) => {
          try {
            ...
            // 加载插件
            const suchPluginManager = new SuchMusicPluginHost()
            await suchPluginManager.loadPlugin(fullPath, new Logger(pluginId))

            // 获取插件信息
            const pluginInfo = suchPluginManager.getPluginInfo()
  ...
  async getPluginsList() {
    ...
    return Object.entries(loadedPlugins).map(([pluginId, manager]) => {
      const suchPluginManager = manager as SuchMusicPluginHost
      return {
        pluginId,
        pluginName: pluginId.split('-')[1] || pluginId,
        pluginInfo: suchPluginManager.getPluginInfo(),
        supportedSources: suchPluginManager.getSupportedSources()
      }
    })
  },
  ...
  async downloadAndAddPlugin(url: string, type: 'lx' | 'cr') {
    try {
      ...
      if (type === 'cr') {
        // Such格式校验：检查是否包含suchmusic关键字
        if (!pluginCode.toLowerCase().includes('suchmusic')) {
          throw new Error('Such插件格式校验失败：代码中未找到suchmusic关键字')
        }
      } else if (type === 'lx') {
        // 洛雪格式校验：检查是否包含lx关键字
        if (!pluginCode.toLowerCase().includes('lx')) {
      ...
      if (transformResult) {
        // 将转换过程中的日志统一输出到控制台，便于调试
        if (transformResult.logs.length) {
          console.group('[SnowdropTransform] 落雪插件在线安装日志')
          for (const log of transformResult.logs) {
            console.log(log)
          }
          console.groupEnd()
