...
export const usePluginStore = defineStore('plugin', () => {
  ...
  const searchMusic = async (
    ...
    try {
      ...
      const list = (result.list || []).map((item: MusicInfo) => ({
        ...item,
      ...
      throw new Error(err.message || '搜索音乐失败')
    }
  }
   ...
   * @returns 更新结果
   */
  const checkPluginUpdate = async (pluginId: string): Promise<PluginUpdateResult> => {
    try {
      const result = await window.electron.ipcRenderer.invoke('plugin:checkUpdate', pluginId)
      return result
    } catch (err: any) {
      return {
        hasUpdate: false,
  ...
  const getMusicUrl = async (
    ...
    try {
      // 1. 如果 musicInfo 指定了 pluginId，优先使用该插件
      if (musicInfo.pluginId) {
      ...
      // 2. 尝试使用当前激活的插件
      if (activePluginId.value && activePluginId.value !== musicInfo.pluginId) {
        const result = await callPluginMethod(
