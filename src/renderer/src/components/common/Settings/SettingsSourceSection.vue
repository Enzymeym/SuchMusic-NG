...
const importOptions: any[] = [
  {
    key: 'import-file',
    label: '本地文件导入',
    icon: renderIcon('mgc_file_new_line')
  },
  {
    key: 'import-url',
    label: '在线 URL 导入',
    icon: renderIcon('mgc_link_line')
  }
]
// URL 导入对话框状态
 ...
 * 统一的文件导入处理
 */
const handleUnifiedImportFile = async () => {
  loading.value = true
  errorMessage.value = null
  try {
    ...
    if (result.plugin) {
      if (result.plugin.type === 'snowdrop') {
        await loadAllPlugins()
 ...
 * 配置 Such 插件
 */
const handleConfigureSuchPlugin = (plugin: Plugin) => {
  currentInitPluginId.value = plugin.id
  showPluginInitModal.value = true
}
...
const handleSetSuchPluginActive = async (plugin: Plugin) => {
  ...
  try {
    ...
    await loadSuchPlugins()
    message.success(`插件 "${plugin.info.name}" 已激活`)
  } catch (err: any) {
    message.error(err.message || '激活插件失败')
  } finally {
    loading.value = false
  }
}
 ...
 * 停用 Such 插件
 */
const handleDeactivateSuchPlugin = async (plugin: Plugin) => {
  loading.value = true
  try {
    await pluginStore.deactivatePlugin(plugin.id)
    await loadSuchPlugins()
    message.success(`插件 "${plugin.info.name}" 已停用`)
  } catch (err: any) {
    message.error(err.message || '停用插件失败')
  } finally {
    loading.value = false
  }
}
 ...
 * 卸载 Such 插件
 */
const handleUninstallSuchPlugin = async (plugin: Plugin) => {
  loading.value = true
  try {
    await pluginStore.uninstallPlugin(plugin.id)
    await loadSuchPlugins()
    message.success(`插件 "${plugin.info.name}" 已卸载`)
  } catch (err: any) {
    message.error(err.message || '卸载插件失败')
  } finally {
    loading.value = false
  }
}
            ...
            >
              <div class="plugin-card-main">
                <div class="plugin-card-left">
                  ...
                  <div class="plugin-line" v-if="plugin.status !== 'error'">支持的音源：</div>
